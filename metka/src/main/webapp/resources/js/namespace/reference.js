(function () {
	...
	var referenceHandlerChooser,
		referenceContainer,
		referenceInput,
		makeReferenceGroupRequest,
	 ...
	 */
	referenceHandlerChooser = function (key, context, dependencyValue, container) {
	...
	referenceContainer = function (field, context, dependencyValue) {
	 ...
	 */
	referenceInput = function (field, context, dependencyValue, container) {
		var reference = null;
		switch (field.type) {
	 ...
	 */
	makeReferenceRequest = function (key, dependencyValue, context, container) {
	 ...
	 * @param context Configuration context
	 */
	makeReferenceGroupRequest = function (request, context, handler) {
		$.ajax({
			type: 'POST',
			headers: {
	 ...
	 */
	handleReferenceInput = function (data, context) {
	 ...
	 */
	createReferenceSelection = function (field, options, includeEmpty, container) {
	 ...
	 * @param container Possible container field key, should only be present on subfields.
	 */
	createReferenceText = function (field, option, container) {
		// Sanity check, if field is subfield but there is no container then nothing can be done.
		if (field.subfield && (container === null || container === '')) {
			return;
		}
